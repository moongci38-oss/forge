...
**Claude Code 설정 (CLI 방식 — 권장):**

> Playwright MCP 대비 **토큰 4x 절감** (114K → 27K), 세션 안정성 향상 (15회 → 50+ 상호작용).

```bash
# CLI 설치
npm install -g @playwright/cli@latest
playwright-cli install --skills   # .claude/skills/playwright-cli/SKILL.md 자동 생성
```

**주요 CLI 명령:**
```bash
playwright-cli open <url>              # 브라우저 열기
playwright-cli snapshot                # 접근성 스냅샷 (YAML)
playwright-cli screenshot --path <p>   # 스크린샷 캡처
playwright-cli resize <w> <h>          # 뷰포트 변경
playwright-cli click <ref>             # 요소 클릭
playwright-cli fill <ref> <text>       # 폼 입력
playwright-cli close                   # 브라우저 종료
```
...
**실전 프롬프트 예시:**
```
playwright-cli로 localhost:3000에서 회원가입 폼을 테스트 데이터로 채우고 제출한 후 성공 메시지를 확인해줘
모바일, 태블릿, 데스크톱 세 가지 화면 크기로 홈페이지 스크린샷을 찍어줘
```

**컨텍스트 관리 전략:** CLI는 YAML 기반 디스크 저장으로 컨텍스트 오염이 없다. MCP 대비 도구 정의 토큰이 불필요하므로 대형 페이지에서도 안정적이다. **서브에이전트에서 Bash(playwright-cli) 도구를 사용하는 패턴**이 메인 컨텍스트 오염을 방지하는 최선 방법이다.
...
model: sonnet
---
playwright-cli 명령으로 웹 인터페이스를 테스트합니다.
```
