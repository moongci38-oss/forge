#!/usr/bin/env bash
# deploy-symlinks.sh — Forge→Forge Dev symlink 일괄 생성/상태 확인
...
#
# forge-workspace.json의 프로젝트 설정을 읽어 개발 프로젝트에
# Forge 산출물 symlink를 일괄 생성한다.
...
usage() {
    cat <<'EOF'
Forge → Forge Dev Symlink Deployer
...
resolve_source() {
    ...
    local source_dir="$FORGE_ROOT/$folder_rel"
...
cmd_list() {
    if [[ ! -f "$WORKSPACE_JSON" ]]; then
        echo -e "${RED}forge-workspace.json not found:${NC} $WORKSPACE_JSON"
        exit 1
    fi
    ...
    if [[ -z "$projects" ]]; then
        echo -e "  ${YELLOW}No projects found in forge-workspace.json${NC}"
        return
    fi
