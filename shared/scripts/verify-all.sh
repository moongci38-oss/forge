...
# Usage: bash scripts/verify-all.sh [rules|skills|forge|all] [--project <name>] [--stage <S1-S4>]
#
...
Usage:
  ...
  verify-all.sh forge --project <name> --stage <S1-S4>   Forge gate check
  verify-all.sh trace --project <name>           Forge Wave 2 traceability check

Options:
  --project <name>    Forge project name (required for forge/trace)
  --stage <S1-S4>     Forge stage (required for forge)

Examples:
  verify-all.sh all
  verify-all.sh rules
  verify-all.sh forge --project baduki --stage S4
...
cmd_forge() {
    ...
    if [[ -z "$project" || -z "$stage" ]]; then
        echo -e "${RED}Error: --project and --stage required for forge check${NC}"
        exit 1
    fi

    run_section "Forge Gate ($project / $stage)" "$SCRIPTS_DIR/forge-gate-check.sh" "$project" "$stage"
}
...
cmd_trace() {
    ...
    run_section "Forge Wave 2 Traceability ($project)" "$SCRIPTS_DIR/forge-wave2-trace.sh" "$project"
}
...
cmd_all() {
    ...
    if [[ -n "$project" ]]; then
        if [[ -n "$stage" ]]; then
            cmd_forge "$project" "$stage"
        fi

        # Wave 2 trace는 S4 산출물이 있을 때만
        if [[ -f "$SCRIPTS_DIR/forge-wave2-trace.sh" ]]; then
            cmd_trace "$project"
        fi
    fi
}
