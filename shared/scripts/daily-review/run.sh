...
RAW_JSON="$FORGE_DIR/01-research/daily/$TARGET_DATE/raw-data.json"
...
"$HOME/.local/bin/claude" -p "/daily-system-review $TARGET_DATE" \
...
if [ "$EXIT_CODE" -eq 0 ]; then
  bash "$SCRIPT_DIR/../tg-send.sh" "✅ *Daily Review 완료* ($TARGET_DATE)\n로그: \`$(basename "$LOG_FILE")\`" 2>/dev/null || true
else
  bash "$SCRIPT_DIR/../tg-send.sh" "❌ *Daily Review 실패* ($TARGET_DATE)\nExit: $EXIT_CODE\n로그: \`$(basename "$LOG_FILE")\`" 2>/dev/null || true
fi
