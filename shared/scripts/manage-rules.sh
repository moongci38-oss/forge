...
_compile_files() {
...

    {
        ...
        echo "> Source: planning/rules-source/"
        echo ""
...
_compile_progressive() {
    ...
    {
        echo "# Forge Pipeline Rules (Compiled $(date +%Y-%m-%d))"
        echo ""
        echo "> Auto-generated by manage-rules.sh build --scope forge"
        echo "> Progressive Disclosure: CRITICAL/HIGH=전문, MEDIUM/LOW=요약+참조"
        echo "> Source: planning/rules-source/forge/"
        echo ""
        ...
        for md_file in "$scope_dir"/*.md; do
            ...
            case "$impact" in
                ...
                MEDIUM|LOW)
                    ...
                    echo "$summary"
                    echo ""
                    echo "> 상세: \`planning/rules-source/forge/$name.md\` 참조"
                    echo ""
                    ;;
            esac
        done
...
cmd_stats() {
    ...
    for f in "$HOME/CLAUDE.md" "$FORGE_ROOT/CLAUDE.md"; do
      [ -f "$f" ] || continue
      local size
      size=$(wc -c < "$f")
      local tokens=$((size / 4))
      ...
      local display
      display=$(realpath --relative-to="$FORGE_ROOT" "$f" 2>/dev/null || basename "$f")
      printf "  %-35s %6d tokens\n" "$display" "$tokens"
    done
...
cmd_sync() {
    ...
    # Sync forge rules as symlinks
    local forge_source
    forge_source="$(eval echo '~/.claude/forge/rules')"
    if [ -d "$forge_source" ]; then
        echo -e "${BLUE}Syncing forge rules (symlinks):${NC}"
        for f in "$forge_source"/forge-*.md; do
            [ -f "$f" ] || continue
            local name
            name=$(basename "$f")
