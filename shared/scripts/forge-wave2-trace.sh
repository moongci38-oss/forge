#!/bin/bash
# Forge Wave 2 트레이서빌리티 자동 검증 스크립트
# S3 기획서에서 요구사항 ID를 추출하고, S4 산출물에서 참조 여부를 검증한다.
# Usage: bash scripts/forge-wave2-trace.sh <project>
# Example: bash scripts/forge-wave2-trace.sh portfolio-admin
...
if [[ -z "$PROJECT" ]]; then
  echo "Usage: bash scripts/forge-wave2-trace.sh <project>"
...
echo "=== Forge Wave 2 트레이서빌리티 검증: $PROJECT ==="
echo ""
echo "S3 기획서: $S3_FILE"
echo ""
