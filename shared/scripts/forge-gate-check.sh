#!/bin/bash
# Forge Gate DoD [AI] 항목 자동 검증 스크립트
# Usage: bash scripts/forge-gate-check.sh <project> <stage>
# Example: bash scripts/forge-gate-check.sh baduki S4
...
if [[ -z "$PROJECT" || -z "$STAGE" ]]; then
  echo "Usage: bash scripts/forge-gate-check.sh <project> <stage>"
  echo "  project: baduki, portfolio-admin, etc."
  echo "  stage: S1, S2, S3, S4"
  exit 1
fi

# forge-workspace.json에서 경로 해석
WORKSPACE_JSON="forge-workspace.json"
if [[ ! -f "$WORKSPACE_JSON" ]]; then
  echo "FAIL: forge-workspace.json not found in $(pwd)"
  exit 1
fi
...
echo "=== Forge Gate Check: $PROJECT / $STAGE ==="
echo ""
...
case $STAGE in
  ...
  S4|s4)
    ...
    # Forge Dev 세션 로드맵
    check_grep "Forge Dev 세션 로드맵" "Forge Dev.*세션\|세션.*로드맵" "$PROJECT_PRODUCT" "*s4-development-plan*.md"
    ;;
...
if [[ $FAIL -gt 0 ]]; then
  ...
  if [[ "$AUTO_PASS" == "true" ]]; then
    case $STAGE in
      ...
      S4|s4) echo "AUTO-PASS SUMMARY: S4 DoD all PASS (${PASS} items). Ready for Forge Dev." ;;
