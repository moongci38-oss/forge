#!/usr/bin/env bash
# Forge ADR Cross-Project Index — ADR 교차 참조 인덱스
# Usage: bash scripts/forge-adr-index.sh {build|search <keyword>}
set -euo pipefail
...
cmd_search() {
  local keyword="${1:-}"
  if [ -z "$keyword" ]; then
    echo -e "${RED}Usage: forge adr <keyword>${NC}"
    exit 1
  fi
...
case "${1:-help}" in
  ...
  *)      echo "Usage: forge-adr-index.sh {build|search <keyword>}" ;;
esac
