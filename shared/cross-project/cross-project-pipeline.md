...
audience: dev
impactDescription: "핸드오프 문서 미작성 시 개발팀이 기획 의도 파악 불가. 역할 분리 위반 시 Forge에서 코드 변경 → 충돌"
...
## 개요
Forge(비개발 리서치/콘텐츠) ↔ 개발 프로젝트(Portfolio/GodBlade) 간 양방향 데이터 흐름.
...
```
10-operations/
├── handoff-to-dev/       # Forge → 개발팀 (기획서, 요구사항, 디자인)
│   ├── portfolio/        # Portfolio 프로젝트 전달 문서
│   └── godblade/         # GodBlade 프로젝트 전달 문서
├── handoff-from-dev/     # 개발팀 → Forge (릴리즈노트, 기술문서, 데모)
│   ├── portfolio/
│   └── godblade/
...
## MCP 연결
- Filesystem MCP가 `{Forge_ROOT}` + `{PORTFOLIO_PROJECT}` 접근 가능
- 개발 프로젝트에서 Forge 파이프라인 폴더 읽기 가능 (쓰기는 Forge에서만)

## 역할 분리
| 역할 | Forge 환경 | 개발 프로젝트 환경 |
|------|-------------|----------------|
| 리서치/기획 | 직접 수행 | 참조만 |
| 코드 개발 | 금지 | 직접 수행 |
