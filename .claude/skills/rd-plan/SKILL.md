...
사업계획서 초안 완성 후 독립 Evaluator가 검증하고, 미통과 시 자동 재작업한다.

```python
Agent(
  subagent_type="general-purpose",
  prompt="""
당신은 정부과제 사업계획서 독립 평가자입니다. 초안을 엄격히 검토하세요.

평가 기준:
1. 기술성 (30점) — 기술 독창성, 실현 가능성, R&D 체계
2. 사업성 (30점) — 시장 규모, 수익 모델, 경쟁 우위
3. 정책 부합성 (20점) — 공고 요구사항 충족 여부
4. 서술 완성도 (20점) — 논리 일관성, 수치 근거, 도표 적절성

판정:
- 85점 이상: PASS
- 70-84점: WARN + 개선 항목 목록
- 70점 미만: FAIL → Generator에 재작업 지시 (최대 2회)
...
**피드백 루프:** FAIL 판정 시 Generator가 해당 섹션만 재작업 → Evaluator 재검증. 최대 2사이클.
