...
3. **Unity 클라이언트 .cs 수정이 포함된 경우** (필수 순서):
   1. `{project_root}/.claude/reference/pre-modification-analysis-detail.md` **Read** (파일을 직접 읽어야 함)
   2. 해당 파일의 4단계 지침(Step 0~4)을 순서대로 수행
   3. 분석 결과를 `{project_root}/.claude/state/current-analysis.md`에 저장
...
2. `{project_root}/.claude/rules/pge-game-evaluator-rubric.md` 읽기 (+ 링크된 rubric-detail.md도 Read)
