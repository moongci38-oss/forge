...
1. `{project_root}/.claude/reference/` 존재 확인 → 태스크 유형에 맞는 파일 Read (파일 기반 통신 프로토콜의 로딩 표 참조)
2. 작업 요구사항 분석
...
1. `{project_root}/.claude/state/PGE_SPEC.md` 읽기
2. PGE_SPEC.md의 "## 참조 컨텍스트" 섹션에 명시된 reference 파일들을 직접 Read
3. 계획에 따라 산출물 생성/구현 — **반드시 reference의 패턴/규칙을 준수**
4. Rubric 기준을 의식하며 생성. 목표: **"museum quality"** (라이브러리 기본값·AI 슬롭 패턴 금지)
5. **QA 핸드오프 전 자기검토** — 아래 체크 후 Evaluator에게 전달:
   - [ ] Rubric 불합격 조건 직접 확인
   - [ ] "이 정도면 됐다" 자기합리화 없음
   - [ ] 실제로 실행/렌더링되는지 확인
...
1. `.claude/rules/pge-game-evaluator-rubric.md` 읽기
2. `.claude/reference/pge-game-evaluator-rubric-detail.md` 읽기
...
Planner는 시작 시 `{project_root}/.claude/reference/` 를 확인하고 태스크 유형에 맞는 파일을 읽는다.
읽은 파일 목록과 핵심 내용은 `PGE_SPEC.md` 상단 "## 참조 컨텍스트" 섹션에 기록한다.

| 태스크 유형 | 읽을 파일 |
|------------|---------|
| 기능 구현 / 버그 수정 | `key-file-map.md`, `code-snippets.md`, `golden-rules.md` |
| UI / 연출 (Unity) | `key-file-map.md`, `code-snippets.md`, `pre-modification-analysis-detail.md` |
| 프로토콜 / 네트워크 | `key-file-map.md`, `protocol-ranges.md`, `tech-stack.md` |
| 빌드 / 배포 | `build-commands.md`, `dependency-order.md` |

Generator와 Evaluator는 실행 전 반드시 `PGE_SPEC.md`의 "## 참조 컨텍스트" 섹션을 읽고 명시된 파일들을 직접 Read한다.
Evaluator는 추가로 `.claude/rules/pge-game-evaluator-rubric.md`와 `.claude/reference/pge-game-evaluator-rubric-detail.md`를 읽는다.
