...
style-guide.md의 `Rendering Level` 필드를 읽어 프롬프트에 레벨별 키워드를 자동 주입한다.
**레벨 정의**: `shared/design-tokens/rendering-levels.md`

슬롯 10(스타일)에 레벨 키워드를 삽입하고, 슬롯 5(환경)에 배경 스타일을 반영한다.

| Level | 슬롯 10 추가 키워드 | 슬롯 5 배경 스타일 |
|:-----:|-----------------|---------------|
| L2 | soft shadow, subtle gradient | clean white background |
| L3 | glassmorphism, backdrop blur, glow arrows, layered shadows | soft gradient + grid pattern |
| L3.5a | isometric, 3D blocks, dimetric | isometric grid, top-left light |
| L3.5b | editorial infographic, data viz, metric callout | structured grid, white |
| L3.5c | neon glow, blueprint grid, HUD labels | dark (#0a0a1a), circuit pattern |
| L4 | cinematic 3D, metallic, HDRI lighting, DOF | dark + volumetric light |
