...
**UI 레이아웃 분석 (기본)**:
```
다음 게임 스크린샷의 UI를 개별 컴포넌트로 분해해주세요.
...
## 출력 형식
| # | 컴포넌트명 | 타입 | 반복 수 | 크기 (비율) | 색상 (Hex) | 텍스트 | 구현 노트 |
|---|---------|------|:------:|:----------:|:---------:|-------|---------|
| 1 | 메인 배경 | Background | 1 | 100% | #1A1A2E | - | |
| 2 | 확인 버튼 | Button | 3 | 80%×8% | #FF6B35 | "확인" | 동일 에셋, 세로 320px 간격 |
| 3 | 닫기 버튼 | Button | 1 | 40×40 | transparent | - | 우상단 X 아이콘 |

+ 컬러 팔레트(Hex 값)도 추출해주세요.
+ Unity UGUI 구현 방법(Canvas 설정, Layout Group, Anchor)도 포함해주세요.
```
...
> 4가지 모드 중 컨텍스트로 자동 판별한다. 사용자에게 묻지 않는다.
...
**기본 모드**: 일반 분석 리포트 (Step 5 기본 형식 사용)

**시안 분석 모드** (`--mockup`): 우리 디자인 시안/목업 분석 전용

> 시안은 디자인 의도가 담긴 확정 자료이므로, 경쟁작 분석과 달리 모든 값을 `(확정)` 태그로 추출한다.
...
> 각 UI 요소/색상/간격/크기를 Element Task Doc의 대응 섹션에 직접 매핑한다.
...
> 토큰명은 프로젝트 `style-guide.md` 기존 토큰과 매칭을 먼저 시도한다.
> 매칭 토큰이 없으면 `--color-{용도}` 패턴으로 신규 토큰 생성.
> 모든 값에 `(확정)` — 추정이 아닌 디자인 의도.

#### Section 7 — Prefab 계층 구조 (확정)

트리 구조 + 컴포넌트 테이블 (Task Doc 모드와 동일 형식, 값은 `(확정)`)
...
> 스크린샷에서 관찰된 레이아웃을 기반으로 Anchor/Pivot을 추정한다.
...
| 요소 | 토큰명 | 값 | 출처 |
|------|--------|-----|------|
| [배경] | `--color-bg` | #FFFFFF | 스크린샷 분석 |
| [주요 버튼] | `--color-primary` | #0095F6 | 스크린샷 분석 |

> 토큰명은 `--color-{용도}`, `--space-{크기}`, `--font-{레벨}` 패턴으로 자동 네이밍한다.
> 추출된 Hex 값에 가장 가까운 시맨틱 용도를 판단하여 토큰명을 부여한다.
...
### Step 5: 결과 구조화 (필수 출력 형식)

분석 결과를 아래 3개 테이블로 **반드시** 출력한다. 테이블이 없으면 FAIL.
...
> 모든 시각 요소를 고유 컴포넌트 단위로 분해. 누락 금지.

| # | 컴포넌트명 | 타입 | 반복 | Z순서 | 크기 (비율) | 색상 (Hex) | 텍스트 | 구현 노트 |
|---|---------|------|:---:|:----:|:----------:|:---------:|-------|---------|
| 1 | 전체 배경 | Background | 1 | 0 | 100% | #1A1A2E | - | |
| 2 | 상단 바 | Container | 1 | 1 | 100%×8% | #2D2D44 | - | Anchor: stretch-top |
| 3 | 닫기 버튼 | Button | 1 | 2 | 40×40 | transparent | X | 우상단, 아이콘만 |
| 4 | 확인 버튼 | Button | 3 | 1 | 80%×50px | #FF6B35 | "확인" | 동일 에셋, 세로 반복 |
| 5 | 반투명 오버레이 | Overlay | 1 | 3 | 100% | #000000 50% | - | alpha=0.5 |
...
| **Phase 2 (Element Task Doc)** | Complex UI 요소 상세 명세 시 | 스크린샷 분석 (Task Doc 모드) → Section 7 Prefab 계층 + Section 10 디자인 토큰 + Section 16 레퍼런스 바인딩 직접 출력 |
...
3. Task Doc 모드 적용 시 Section 7 / Section 10 / Section 16 형식으로 직접 출력한다
4. 기본 모드와 Task Doc 모드 사이에서 모드 선택을 사용자에게 묻지 않는다 — 컨텍스트로 자동 판단한다
5. 분석 전 "X 분석 → [기본/Task Doc] 모드로 실행합니다" 한 줄 선언 후 실행한다
