# SIGIL Pipeline Rules (Compiled 2026-03-17)
...
pipeline-orchestrator는 S3 에이전트 회의 시작 전 S2 gate-log에서 디렉션 파일 경로를 읽고, 5축 요약을 에이전트 프롬프트에 주입한다.

**프롬프트 주입 형식 (~5줄, 축당 1문장):**

```
## S2 기획 디렉션 (요약)
1. 전략 방향: {한 문장}
2. 경험 원칙: {한 문장}
3. 범위 경계: Do {N}개 / Don't {N}개 — 상세: {파일 경로}
4. 품질 기준: {한 문장}
5. 벤치마크: {레퍼런스 이름들}
```

에이전트가 상세 내용이 필요하면 Read 도구로 전체 디렉션 파일을 참조한다.

### 디렉션 탈락 필터 (Dropout Filter)

에이전트 A/B/C가 자유 초안을 작성한 후, 비교 전에 **범위 경계 Don't 태그 위반 여부**를 필터링한다:
...
디렉션 탈락 필터 (Don't 태그 매칭)
    ...
    ↓
생존 초안만 기존 기준으로 비교 (아키텍처, 성능, UX 등)
```
...
|----------|---------|---------|:----:|----------|
| **S2 디렉션 탈락 상태** (신규) | PASS / FAIL({태그}) | PASS / FAIL({태그}) | — | Don't 위반 초안은 비교에서 제외 |

### 디렉션 vs 기술 최적해 충돌

| 상황 | 행동 |
|------|------|
| Don't 항목이 기술적으로 필수 | **[STOP]** Human에게 "디렉션 Don't `{태그}`가 기술적으로 필요합니다" 보고 |
| 전략 방향과 기술 최적해 불일치 | 회의 보고서에 기록 + Human 판단 요청 |
| 모든 초안이 같은 Don't 위반 | **[STOP]** Human에게 "범위 경계 재검토 필요" 보고 |
...
12. S3 에이전트 프롬프트에 S2 디렉션 5축 요약 (~5줄)을 인라인 주입한다
13. 에이전트 초안 비교 전 Don't 태그 탈락 필터를 실행한다
14. Don't 항목이 기술적으로 필수인 경우 [STOP] Human에게 보고한다

## Iron Laws

- **S3-IRON-1**: 단일 에이전트 초안만으로 기획서를 확정하지 않는다 (에이전트 회의 필수)
- **S3-IRON-2**: .pptx 없이 기획서 승인을 진행하지 않는다
