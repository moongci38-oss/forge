...
description: |
  Forge S4 Wave 1 기획 패키지 작성 전문 에이전트.
...
| 3 | 개발 계획 | `*-s4-development-plan.md` | 기술 스택 + C4 아키텍처 + ADR + 로드맵 + WBS + Forge Dev 세션 로드맵 + **테스트 전략**(피라미드/커버리지/도구/파일구조) |
...
4. **[3] 개발 계획**: [1]+[2] 기반 기술 스택 → C4 아키텍처 → ADR → 로드맵 → WBS → Forge Dev 세션 로드맵 → **테스트 전략**(피라미드/커버리지/도구/파일구조)
