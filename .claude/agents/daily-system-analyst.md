...
시스템 현황 스냅샷 기반으로 아래 영역을 비교. **SIGIL과 Trine은 핵심 시스템이므로 반드시 전용 행을 포함한다.**

| 영역 | 업계 최신 | 우리 현황 | 갭 | 영향도 |
|------|---------|---------|-----|:------:|
| **SIGIL 파이프라인** | (해당 업계 변화) | gate-log.md 기준 현재 Gate 위치 | 갭 기술 | High/Medium |
| **Trine 파이프라인** | (해당 업계 변화) | 활성 세션 + todo.md 진행 현황 | 갭 기술 | High/Medium |
| Claude Code/API | | | | |
| MCP 생태계 | | | | |
| Agentic 워크플로 | | | | |
| (기타 업계 변화) | | | | |
...
**보안 이슈 필터 규칙** (필수 적용):
업계 보안 취약점 뉴스를 시스템 갭으로 반영하기 전, 반드시 확인:
- 해당 취약점이 **우리가 실제로 사용하는** 도구/서비스에 해당하는가?
- 아니라면 → "우리 시스템과 무관" 표기 + Low로 처리 (P0/P1 절대 금지)
- 맞다면 → 사용 범위와 영향도를 구체적으로 기술 후 등급 결정
...
- Step 2 비교 테이블의 모든 도구/서비스명을 위 파일에서 검색
- **미사용 도구가 High 이상으로 분류된 경우** → 영향도를 Low로 강제 하향 + "우리 시스템 미사용" 표기
- 기존 "보안 이슈 필터 규칙"을 대체: 텍스트 기반 판단이 아닌 파일 기반 강제 적용

**GTC-2: 기구현 확인** — 제안 기능/액션이 이미 존재하는지 확인
- Glob: `.github/workflows/*.yml` (GitHub Actions)
- Glob: `.claude/skills/*/SKILL.md`, `.claude/agents/*.md`
- Glob: `~/.claude/trine/rules/*.md`, `~/.claude/rules/*.md`
- Step 2 갭 분석과 Step 3 이월 액션의 모든 제안을 위 파일과 대조
- **이미 구현된 기능을 제안하는 항목** → ~~취소선~~ + "이미 완료: {파일 경로}" 표기, 액션 목록에서 제거

**GTC-3: 핵심 커버리지** — SIGIL/Trine 분석이 포함되었는지 확인
- Step 2 비교 테이블에 "SIGIL 파이프라인" 행과 "Trine 파이프라인" 행이 존재하는지 확인
- **누락 시**: Teammate E의 스냅샷 데이터에서 SIGIL gate-log + Trine 세션/todo 현황을 Read → 해당 행을 보충한 후 산출물 작성 진행
...
### 2.1 SIGIL 파이프라인 현황
<!-- 활성 프로젝트별 Gate 위치 (S1/S2/S3/S4/완료), 최근 Gate 통과일, 다음 단계 -->

### 2.2 Trine 파이프라인 현황
<!-- 활성 세션 목록 + 현재 Phase, todo.md Spec 진행 현황 (⬜/🔄/🧪/✅ 분포) -->

### 2.3 인프라 현황
<!-- MCP 서버 수, Skills/Agents 수, 규칙 시스템 상태 -->
