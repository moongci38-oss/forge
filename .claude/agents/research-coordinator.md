...
Forge S1 리서치의 전략을 수립하고 리서치 에이전트/도구를 조율한다.
...
- **신뢰도 높은 소스 조사**: Brave Search (도메인 필터: `.edu`, `.org`, `.gov`)
- **수치 검증**: fact-checker 스폰 (Brave Search로 공식 소스 교차 검증)
- **데이터 분석**: `data:data-exploration` + `data:statistical-analysis`
- **로컬/지역 조사**: Brave Search (local search)
