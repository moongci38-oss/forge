...
| 2 | **개발 계획** | s4-development-plan.md | 기술 스택 + 아키텍처 + ADR + 세션 로드맵 + 로드맵(Now/Next/Later) + WBS(태스크 분해/규모 추정) | Phase 1 컨텍스트 |
| 3 | **UI/UX 기획서** | s4-uiux-spec.md | 와이어프레임 + 컴포넌트 스펙 + 인터랙션 패턴 + 디자인 가이드 | Phase 2 Spec UI |
| 4 | **테스트 전략서** | s4-test-strategy.md | 테스트 계층/비율 + FE/BE 도구 + 시딩 전략 + 커버리지 목표 | Phase 3 Check |

> **흡수된 산출물**: 사이트맵 → 상세 기획서에 통합, 로드맵/WBS → 개발 계획에 통합. S4 Gate 통과 시 todo.md도 자동 생성된다.
