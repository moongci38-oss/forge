...
**용도**: Trine+Forge 파이프라인의 작업/버그/기능을 Notion에서 통합 관리하는 구조 정의
...
> **Source of Truth**: `forge-workspace.json`의 `notionDBs` 섹션에서 참조.
> 아래 URL은 참조용이며, 변경 시 반드시 `forge-workspace.json`을 업데이트한다.
...
| 시작일 | Date | — |
| 담당자 | Person | — |
| Forge 단계 | Select | S1(파랑) / S2(초록) / S3(노랑) / S4(주황) / Trine(보라) / 완료(회색) |
...
Notion MCP 미연결 시 `docs/planning/active/forge/todo.md` 사용.
