...
**용도**: 각 게이트에서 산출물 완성도를 검증하는 체크리스트
**적용**: 모든 SIGIL 프로젝트

> `[STOP]` = Human 승인 대기 / `[AUTO-PASS]` = AI 자동 검증 + 알림 / `[AI]` = AI 자동 완료 / `[Human]` = Human 실행

---

## S1 Research DoD

> `[AUTO-PASS]` — DoD 전항목 자동 검증 통과 시 S2로 자동 진행
...
> 5축 모두 PASS여야 S2 Gate 통과 가능

| # | 축 | PASS 기준 | 결과 |
|:-:|---|---------|:----:|
| 1 | 전략 방향 | "A > B" 또는 "A, not B" 형식 포함 | ☐ |
| 2 | 경험 원칙 | 수치/시간/횟수 포함 | ☐ |
| 3 | 범위 경계 | Do 2개+ / Don't 2개+ (태그 형식) | ☐ |
| 4 | 품질 기준 | 측정 가능한 값 포함 (Go/No-Go와 구별) | ☐ |
| 5 | 벤치마크 | 레퍼런스 1-3개 + 참조 이유 | ☐ |

> **Axis 4 vs Go/No-Go**: Go/No-Go = 실현 가능성 ("할 수 있는가?"), Axis 4 = 품질 비타협선 ("어느 수준으로 해야 하는가?")

### 추가 항목
- [ ] `[AI]` 제품 컨셉 문서 (핵심 기능, 타겟, 차별점)
- [ ] `[AI]` 관리자 우선순위 결정 (서비스 > 관리자 / 관리자 ≥ 서비스 / 동등)
- [ ] `[AI]` 산출물: `{folderMap.product}/{project}/YYYY-MM-DD-s2-concept.md`
- [ ] `[AI]` 기획 디렉션 5축 작성 + PASS/FAIL 검증 완료
- [ ] `[Human]` 디렉션 5축 최종 확정 (S2 [STOP] 게이트에서 승인)
...
> `[AUTO-PASS]` — Wave 2+3 + gate-check 통과 시 Trine으로 자동 진행. FAIL 시 [STOP] 에스컬레이션
...
- [ ] `[AUTO-PASS]` 자동 검증 통과 시 Trine 세션 시작 안내

---

## 게이트 로그

각 게이트([STOP] 또는 [AUTO-PASS]) 통과 시 프로젝트 폴더의 `gate-log.md`에 자동 기록한다.

| 기록 항목 | 설명 |
|---------|------|
| Stage | S1 / S2 / S3 / S4 |
| 결과 | ✅ PASS / ✅ AUTO / 🔄 수정 후 통과 / ❌ 반려 |
| 일자 | YYYY-MM-DD |
| 세션 | 세션 번호 (같은 날 같은 대화 세션 = 동일 번호) |
| 조건 | 충족된 DoD 요약 또는 Go/No-Go 점수 |
| humanDecision | `approved` / `modified` / `rejected` — [STOP] 게이트에서 Human 결정 기록 ([AUTO-PASS]는 `auto`) |
...
1. 각 게이트([STOP] 또는 [AUTO-PASS]) 진입 전 해당 Stage DoD를 점검한다
