...
## Inspector Reference 연동

S4 산출물(특히 UI/UX 기획서, 상세 기획서) 작성 시:

1. **읽기**: `docs/references/inspector-reference.md`를 먼저 읽고 유사 컴포넌트의 검증된 값을 복사하여 시작
2. **쓰기**: 새로운 UI/연출 Spec에 Inspector Values 섹션을 포함하여 AI 초안값을 명시
3. **교정**: Human이 에디터에서 확인 후 교정값을 inspector-reference.md에 커밋

> inspector-reference.md가 없으면 `forge/planning/templates/inspector-reference-template.md`에서 복사하여 생성한다.
...
- UI/연출 Spec 작성 시 inspector-reference.md를 참조한다
