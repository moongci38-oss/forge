...
> 모든 경로는 `forge-workspace.json`의 `folderMap`에서 해석한다.
...
| Handoff 문서 | `handoff` | `{target-project}/YYYY-MM-DD-forge-handoff.md` |
...
1. **파이프라인 시작 시 `forge-workspace.json`을 읽어 경로를 해석한다** — 없으면 [STOP]
...
6. S4 완료 후 Forge Dev Handoff 문서를 자동 생성하고 진입을 안내한다
