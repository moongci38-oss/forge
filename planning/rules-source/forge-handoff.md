---
title: "Forge → Forge Dev 전환"
id: forge-handoff
impact: HIGH
scope: [forge, forge-dev]
tags: [handoff, transition, forge-dev]
requires: [forge-s4-planning]
section: forge-pipeline
audience: dev
impactDescription: "Handoff 문서 미생성 시 Forge Dev에서 S4 산출물 참조 경로 불명 → 기획-개발 단절. 기술 스택/ADR 미전달 시 재의사결정 필요"
enforcement: rigid
---

# Phase 5: Handoff + Project Setup

> **통합 파이프라인 Phase 매핑**: S4 완료 = Phase 4 완료 → Phase 5 Handoff → Phase 6 Session Setup
> **통합 문서**: `forge/pipeline.md` (Phase 1~12)

## 경로 해석 규칙

모든 경로는 워크스페이스 루트의 `forge-workspace.json`에서 해석한다.

```
경로 해석: {folderMap.product}/{project}/ → 실제 경로 (예: 02-product/projects/portfolio-admin/)
```

> `forge-workspace.json`이 없으면 **[STOP]** — `forge-init` 실행을 안내한다.

## 전환 조건

Phase 6 진입은 아래 **모든 조건**을 충족해야 한다:
...
- **역할**: Forge(설계) → Forge Dev(구현) 간 정보 전달 공식 문서
- **내용**: 산출물 인덱스, 기술 스택, 세션 로드맵, 개발 환경, ADR 요약, 우선순위

## Forge 산출물 → Forge Dev 매핑

| Forge 산출물 | Forge Dev 활용 시점 | 용도 |
|-------------|----------------|------|
...
## 프로젝트 유형별 Forge Dev 대상

| 프로젝트 유형 | Forge Dev 대상 환경 | Spec 템플릿 | 비고 |
|-------------|---------------|-----------|------|
...
`forge-workspace.json`의 `projectType` 필드로 Spec 템플릿을 자동 선택한다:
...
S4 기획 패키지에 포함해야 하는 게임 프로젝트의 시각 자료 최소 기준. `forge-workspace.json`의 `projectScale` 필드로 차등 적용:

#### 규모 분류 기준

| 규모 | 기준 |
|------|------|
| **Small** | Forge Dev 세션 3개 이하, 핵심 시스템 2개 이하 |
| **Large** | Forge Dev 세션 4개 이상, 또는 핵심 시스템 3개 이상 |
...
> `forge-s4-planning.md` → "S4 산출물 시각 자료 기준 (프로젝트 유형별)" 테이블 참조

### 웹/앱 프로젝트 Forge Dev 도구
...
S4 기획 패키지에 포함해야 하는 웹/앱 프로젝트의 시각 자료 최소 기준. `forge-workspace.json`의 `projectScale` 필드로 차등 적용:
...
## Symlink 기반 문서 연동 (Source of Truth: Forge 워크스페이스)

**Forge 워크스페이스가 모든 기획/계획 문서의 유일한 원본**이다. 개발 프로젝트는 symlink로 참조만 한다.

### 원칙

- 기획서/개발계획서 수정은 반드시 Forge 워크스페이스에서 Forge 파이프라인을 통해 수행
- 개발 프로젝트의 `{symlinkBase}/` 폴더에 symlink를 생성 (`forge-workspace.json`의 프로젝트별 설정)
- symlink이므로 Forge 워크스페이스에서 수정하면 개발 프로젝트에 자동 반영

### Symlink 대상 (전체 Forge 산출물)
...
|----------------|-------------|--------------------------|
| `YYYY-MM-DD-forge-handoff.md` | `forge-handoff.md` | `{folderMap.handoff}/{project}/` |
...
| `_assets/*.png` (시안 이미지) | (개별 파일 또는 폴더) | `{folderMap.product}/{project}/_assets/` |
| `video-refs/*.md` (영상 분석) | (개별 파일) | `{folderMap.product}/{project}/docs/assets/video-refs/` |
...
- 예: `2026-03-14-forge-handoff.md` → `forge-handoff.md`
...
개발 프로젝트 내 Forge 산출물은 도메인별로 분리한다:
- S3, S4, Handoff, Gate Log → `docs/planning/active/forge/{도메인}/` (symlink)
- {도메인}: Forge 프로젝트의 기능 도메인 이름 (kebab-case)
- 동일 개발 프로젝트에 여러 Forge 프로젝트 → 도메인으로 구분
- forge-workspace.json 등록 시 symlinkBase에 도메인 경로 포함 필수
...
### 개발 프로젝트 매핑 (`forge-workspace.json`)

```json
"projects": {
  "my-project": {
    "devTarget": "/path/to/dev/project",
    "symlinkBase": "docs/planning/active/forge/my-domain"
  }
}
```
...
Human이 개발 프로젝트 환경으로 이동
    ↓
Forge Dev 자동 발동 (Implicit Entry: .specify/ 디렉토리 감지)
    ↓
Forge Dev Session 1부터 순차 진행
...
## Handoff 산출물 인덱스 필수 카테고리

| 카테고리 | 대상 | 경로 패턴 |
|---------|------|---------|
| S3 기획서 | .md + .pptx | `{product}/{project}/` |
| S4 개발계획 | .md | `{product}/{project}/` |
| **시안 이미지** | Human 컨펌된 UI/연출 목업 | `{product}/{project}/_assets/` |
| **영상 분석** | 레퍼런스 영상 프레임 분석 | `{product}/{project}/docs/assets/video-refs/` |
| Q&A | 질의서 + 확정 답변 | `{product}/{project}/` |
| Gate Log | 게이트 통과 기록 | `{product}/{project}/` |

## Do

- S4 Gate PASS 후 Handoff 문서를 자동 생성한다
- 산출물 인덱스에 모든 S4 산출물 경로를 포함한다
- **산출물 인덱스에 `_assets/` 시안 이미지 목록을 포함한다**
- **각 시안 이미지에 대응하는 S4 태스크/FR 번호를 매핑한다**
- **`video-refs/` 영상 분석 문서가 존재하면 산출물 인덱스에 포함한다**
- **시안 이미지 = Human 승인된 디자인 명세로 취급한다 — Forge Dev Spec에서 각 FR의 시각적 기준으로 참조 필수**
- S3에 관리자 기능이 포함된 경우 관리자 산출물도 Handoff에 포함한다
- Handoff 문서에 Todo Tracker 경로를 포함한다
- Handoff 단계에서 개발 프로젝트에 전체 Forge 산출물 symlink를 생성한다
- 기획서/개발계획서 수정은 반드시 Forge 워크스페이스에서 Forge 파이프라인을 통해 수행한다

## Don't

- Handoff 문서 미생성으로 Forge Dev에 진입하지 않는다
- 산출물 인덱스에 누락이 있는 상태로 Handoff를 완료하지 않는다
- Todo Tracker 미생성 상태로 Forge Dev에 진입하지 않는다
- 시안 이미지(`_assets/`)나 영상 분석(`video-refs/`) 존재 시 Handoff 산출물 인덱스에서 누락하지 않는다
- 개발 프로젝트에서 기획/계획 문서 원본을 직접 수정하지 않는다 (symlink이므로 Forge 워크스페이스에서 수정)
- symlink 없이 문서를 복사하여 개발 프로젝트에 배치하지 않는다
- `forge-workspace.json` 없이 경로를 하드코딩하지 않는다
- symlinkBase를 도메인 없이 forge/ 루트로 설정하지 않는다
- todo.md를 도메인 하위에 생성하지 않는다 (forge/ 루트에 통합)
- todo.md를 symlink으로 생성하지 않는다 (실제 파일 필수)

## AI 행동 규칙

1. **`forge-workspace.json`을 먼저 읽고 경로를 해석한다** — 없으면 [STOP]
2. S4 Gate PASS 확인 후 Handoff 문서를 자동 생성한다
3. Handoff 문서에 산출물 인덱스, 기술 스택, 세션 로드맵을 포함한다
4. `forge-workspace.json`의 프로젝트 매핑으로 개발 프로젝트에 전체 symlink를 생성한다
5. Human에게 Forge Dev 진입 안내를 제공한다 (개발 프로젝트 이동 시 Forge Dev 자동 발동)
6. 기획/계획 문서 수정 요청 시 Forge 워크스페이스의 원본을 수정한다 (symlink 자동 반영)
7. 신규 프로젝트 등록 시 symlinkBase에 도메인 경로를 포함한다
8. S4 Gate PASS 시 기존 todo.md에 Spec 행을 추가한다 (새 파일 생성 아님)

## Iron Laws

- **HANDOFF-IRON-1**: Handoff 문서 미생성 상태로 Forge Dev에 진입하지 않는다
- **HANDOFF-IRON-2**: gate-log.md에 S4 PASS 또는 AUTO 기록이 없으면 Forge Dev 진입을 거부한다

## Rationalization Table

| 합리화 (Thought) | 현실 (Reality) |
|-------------------|---------------|
| "Handoff는 형식적이니 스킵해도 개발 가능하다" | Handoff 없이 Forge Dev에 진입하면 S4 산출물 참조 경로가 불명확하여 기획-개발 단절이 발생한다 |
| "어차피 같은 사람이 하니까 문서 없어도 된다" | 1인이라도 세션 간 컨텍스트가 유실된다. Handoff는 세션 독립성을 보장하는 공식 문서다 |

## Red Flags

- "바로 코딩부터 시작하자..." → STOP. gate-log.md에서 S4 PASS를 확인한다
- "Handoff 문서는 나중에 만들면..." → STOP. Forge Dev 진입 전 Handoff 자동 생성을 실행한다
