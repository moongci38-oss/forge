...
3. 세션 상태 초기화 (`node ~/.claude/forge/scripts/session-state.mjs init --name <name>`)
...
> 기획서의 모호한 요구사항을 구현 전에 해소. → `forge-requirements-analysis.md` 규칙 참조.
...
| 기존 (사후 검증) | 추가 (사전 예방) |
|-----------------|----------------|
| Check 3.8 `/forge-check-security` | `security-guidance` 플러그인 — 코드 작성 시점에 예방적 보안 패턴 경고 |
...
3. **결과 분기**:
   ...
   - ❌ FAIL → **Check 6.5**: GitHub Issue 자동 생성 (`integration-failure` + `forge-phase-5` 라벨) → AI가 이슈 분석 + 수정 → develop 재push
...
> `/forge-release` 커맨드로 진입. `release-staging.yml`이 릴리스 브랜치 생성부터 Release PR까지 자동화.

1. Human이 `/forge-release {version}` 실행 (예: `/forge-release 1.2.0`)
...
3. **결과 분기**:
   - ✅ PASS → Phase 7 완료. todo.md 상태 갱신 (`forge-pm-updater`)
   - ❌ FAIL → **Check 8.5**: Human이 `/forge-rollback` 실행:
     - **L1 Quick Revert**: `git revert` — 최근 커밋만 되돌리기 (< 30분)
     - **L2 Release Revert**: 이전 릴리스 태그로 재배포 (< 2시간)
     - **L3 Hotfix Forward**: `hotfix/*` 브랜치에서 Forge Dev Hotfix 플로우 재진입 (> 2시간)
...
| `forge-pm-updater` | 에이전트 | todo.md + Notion 상태 갱신 |
...
### 비-Forge Dev 세션에서 활용 가능한 superpowers 스킬 (파이프라인 외)

| 스킬 | 활용 시점 |
|------|----------|
| `brainstorming` | 기획서 없이 아이디어에서 설계를 시작할 때 (Forge 이전 단계) |
