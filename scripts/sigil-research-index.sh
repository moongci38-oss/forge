#!/usr/bin/env bash
# SIGIL Research Index — S1 리서치 인덱스 빌드 + 검색
# Usage: bash scripts/sigil-research-index.sh {build|search <keyword>}
set -euo pipefail
...
cmd_search() {
  local keyword="${1:-}"
  if [ -z "$keyword" ]; then
    echo -e "${RED}Usage: sigil research <keyword>${NC}"
    exit 1
  fi
...
case "${1:-help}" in
  ...
  *)      echo "Usage: sigil-research-index.sh {build|search <keyword>}" ;;
esac
