#!/usr/bin/env bash
# SIGIL 통합 CLI — 기존 스크립트를 서브커맨드로 래핑
set -euo pipefail
...
usage() {
  echo -e "${BOLD}SIGIL CLI — Strategy & Idea Generation Intelligent Loop${NC}"
  echo ""
  echo "Usage: sigil <command> [options]"
  echo ""
  echo "Commands:"
  echo "  init                    Validate sigil-workspace.json"
  ...
  echo "  verify                  Run all SIGIL verifications"
}

cmd_status() {
  local target_project="${1:-}"
  local ws_json="$SCRIPT_DIR/../sigil-workspace.json"
  ...
  echo -e "${BOLD}=== SIGIL Project Status ===${NC}"
  echo ""

  local product_dir
  product_dir="$(json_get "$ws_json" "folderMap.product")"

  for proj_dir in "$SCRIPT_DIR/../$product_dir"/*/; do
    [ ! -d "$proj_dir" ] && continue
    local proj_name
...
main() {
  ...
  case "$cmd" in
    init)     bash "$SCRIPT_DIR/sigil-validate-workspace.sh" "$@" ;;
    gate)     bash "$SCRIPT_DIR/sigil-gate-check.sh" "$@" ;;
    trace)    bash "$SCRIPT_DIR/sigil-wave2-trace.sh" "$@" ;;
    deploy)   bash "$SCRIPT_DIR/deploy-symlinks.sh" deploy "$@" ;;
    status)   cmd_status "$@" ;;
    metrics)  bash "$SCRIPT_DIR/sigil-metrics.sh" "$@" ;;
    research) bash "$SCRIPT_DIR/sigil-research-index.sh" search "$@" ;;
    adr)      bash "$SCRIPT_DIR/sigil-adr-index.sh" search "$@" ;;
    verify)   bash "$SCRIPT_DIR/verify-all.sh" all --sigil-only "$@" ;;
    --help|-h|help) usage ;;
    *)
